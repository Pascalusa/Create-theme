 ...
 */
class RservationClient {
	private $rservation_client_options;

	public function __construct() {
		add_action( 'admin_menu', array( $this, 'rservation_client_add_plugin_page' ) );
		add_action( 'admin_init', array( $this, 'rservation_client_page_init' ) );
	}

	public function rservation_client_add_plugin_page() {
		add_menu_page(
			'Réservation', // page_title
			'Réservation', // menu_title
			'manage_options', // capability
			'rservation-client', // menu_slug
			array( $this, 'rservation_client_create_admin_page' ), // function
			'dashicons-star-filled', // icon_url
			3 // position
		);
	}

	public function rservation_client_create_admin_page() {
		$this->rservation_client_options = get_option( 'rservation_client_option_name' ); ?>

		<div class="wrap">
      
      <h5 style="margin-bottom: 30px; margin-top: 30px;">Réservation client :</h5>
...
//     $del = $_POST['del'];
//     $del2 = implode (",",$del);
//     $result = $sql->query("DELETE from client WHERE id in ($del2)");               
//  }
 ...
 echo '<table class="striped centered responsive-table">
            <thead>
                  <tr>    
                      ...
                      <th>Jours</td>
                      <th>Horaires</td>
                      <th>Point d\'enlèvement</td>
                      <th>9.90</td>
                      <th>Lunch Bowl + boisson</td>                   
                  </tr>
            </thead>
            <tbody>' ;
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
        ...
        echo '<td>'.$row["lunch_bowl"].'</td>' ;
        echo '<td>'.$row["lunch_boissons"].'</td></tr>' ;
//         echo '<td><div class="switch"><label> Off <input name="del[]" value='.$row["id"].' type="checkbox"><span class="lever"></span> On </label></div> 
//         <input placeholder="Placeholder" name="submit" value="submit" type="submit" class="validate"></td></tr>' ;
    }
} else {
    echo "0 résultats dans la base de donnée";
}
...
?>
			...
			<form method="post" action="options.php">
				<?php
					settings_fields( 'rservation_client_option_group' );
					do_settings_sections( 'rservation-client-admin' );
					submit_button();
				?>
			</form>
		</div>
	...
	public function rservation_client_page_init() {
		register_setting(
			'rservation_client_option_group', // option_group
			'rservation_client_option_name', // option_name
			array( $this, 'rservation_client_sanitize' ) // sanitize_callback
		);
	}

	public function rservation_client_sanitize($input) {
...
if ( is_admin() )
	$rservation_client = new RservationClient();

/* 
 * Retrieve this value with:
 * $rservation_client_options = get_option( 'rservation_client_option_name' ); // Array of All Options
 * $text_0 = $rservation_client_options['text_0']; // text
 */
...
/*************************************Custom post type DELETE COMMANDE************************************ */
 add_action( 'init', 'codex_reservation_init' );

function codex_reservation_init() {
	$labels = array(
 		'name'               => _x( 'Supprimer une commande', 'post type general name', 'your-plugin-textdomain' ),
 		'singular_name'      => _x( 'Supprimer une commande', 'post type singular name', 'your-plugin-textdomain' ),
// 		'menu_name'          => _x( 'Supprimer une commande', 'admin menu', 'your-plugin-textdomain' ),
...
/*************************************Custom post type DELETE COMMANDE FIELD id************************************ */
/**
 * Generated by the WordPress Meta Box generator
 * at http://jeremyhixon.com/tool/wordpress-meta-box-generator/
 */

function delete__get_meta( $value ) {
	global $post;
...
add_action( 'add_meta_boxes', 'delete__add_meta_box' );

function delete__html( $post) {
	wp_nonce_field( '_delete__nonce', 'delete__nonce' ); ?>

	<p>
		<label for="delete__delete_id"><?php _e( 'delete id', 'delete_' ); ?></label><br>
		<input type="text" name="delete__delete_id" id="delete__delete_id" value="<?php echo delete__get_meta( 'delete__delete_id' ); ?>">
	</p><?php
}

function delete__save( $post_id ) {
	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) return;
	if ( ! isset( $_POST['delete__nonce'] ) || ! wp_verify_nonce( $_POST['delete__nonce'], '_delete__nonce' ) ) return;
	if ( ! current_user_can( 'edit_post', $post_id ) ) return;

	if ( isset( $_POST['delete__delete_id'] ) )
		update_post_meta( $post_id, 'delete__delete_id', esc_attr( $_POST['delete__delete_id'] ) );
}
add_action( 'save_post', 'delete__save' );

/*
	Usage: delete__get_meta( 'delete__delete_id' )
*/
...
// http://wp-boilerplates.mathieuleroi.com/page/3/
